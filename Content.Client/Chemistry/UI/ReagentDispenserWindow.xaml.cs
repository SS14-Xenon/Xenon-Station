...
namespace Content.Client.Chemistry.UI
{
    ...
    public sealed partial class ReagentDispenserWindow : FancyWindow
    {
        ...
        public void UpdateContainerInfo(ReagentDispenserBoundUserInterfaceState state)
        {
            ...
            var containerPh = GetContainerPH(state.OutputContainer);
            ContainerInfoFill.Text = Loc.GetString("reagent-dispenser-window-fill-with-ph-label-text",
                ("currentVolume", state.OutputContainer.CurrentVolume),
                ("maxVolume", state.OutputContainer.MaxVolume),
                ("ph", containerPh.ToString("0.0")));
            ContainerInfo.Children.Add(new Label
            {
                Text = Loc.GetString("reagent-dispenser-window-solution-ph-label-text",
                    ("ph", containerPh.ToString("0.0"))),
        ...
        private float GetContainerPH(ContainerInfo container)
        {
            if (container.SolutionPH is { } solutionPH)
                return solutionPH;

            if (container.Reagents is null)
                return ChemistryPH.NeutralPH;

            var solution = new Solution(container.Reagents, false);
            return ChemistryPH.GetSolutionPH(solution, _prototypeManager);
        }
    }
}