...
public sealed class BloodWormSystem : EntitySystem
{
    ...
    public override void Update(float frameTime)
    {
        ...
        while (query.MoveNext(out var uid, out var comp))
        {
            ...
            if (comp.LeaveHostReagents.Count > 0
                && _solution.ResolveSolution(host, hostBloodstream.BloodSolutionName,
                    ref hostBloodstream.BloodSolution, out var bloodSol))
            {
                foreach (var reagent in comp.LeaveHostReagents)
                {
                    if (bloodSol.GetTotalPrototypeQuantity(reagent) > 0)
                    {
    ...
    private bool EnterHost(EntityUid worm, EntityUid host, BloodWormComponent comp)
    {
        ...
        {
            hostComp.HadBloodWormLanguage = hostLanguages.SpokenLanguages.Contains("BloodWorm");
            if (!hostComp.HadBloodWormLanguage)
                _language.AddLanguage(host, "BloodWorm", true, false);
        }
    ...
    private void LeaveHost(EntityUid worm, BloodWormComponent comp, bool transferMindToWorm)
    {
        ...
        if (TryComp(host, out BloodWormHostComponent? hostComp))
        {
            ...
            if (!hostComp.HadBloodWormLanguage)
            {
                if (TryComp(host, out LanguageKnowledgeComponent? knowledge) &&
                    knowledge.SpokenLanguages.Contains("BloodWorm"))
                    _language.RemoveLanguage((host, knowledge), "BloodWorm", true, false);

                if (TryComp(host, out LanguageSpeakerComponent? hostLanguages) &&
                    hostLanguages.CurrentLanguage == "BloodWorm")
                    _language.SetLanguage(host, "Universal");
            }
